"""
Smart Image Tagger — Flask Backend
Deploy to Azure App Service (Free F1 tier or above)

The Azure AI Vision key/endpoint are read from environment variables
(App Service -> Configuration -> Application settings). They are never
...
# ── Azure Vision Config (set in Azure Portal -> Configuration -> App Settings)
...
def analyze():
    ...
    api_url = (
        ...
        f"?api-version=2023-02-01-preview&features=Caption,Tags,Read"
    )
...
@app.route("/health")
def health():
    """Simple health check used by Azure App Service / monitoring."""
